...
extern "C" __global__ void __raygen__main()
{
    ...
    for (int i = 0; i < MAX_TRACE_OPS; ++i)
    {
        ...
        for (uint32_t emitter_index = 0; emitter_index < params.emitters.count; ++emitter_index)
        {
            ...
            if (light_sample.sampling_pdf == 0.0f)
                continue;

            BSDFEvalResult bsdf_eval = bsdf->evalBSDF(si, light_sample.direction_to_light, +BSDFComponentFlag::DiffuseReflection);
            if (glm::dot(bsdf_eval.bsdf_value, bsdf_eval.bsdf_value) < 1e-12f)
                continue;

            float tmax = light_sample.distance_to_light;
            if (!has_flag(emitter->flags, EmitterFlag::DistantEmitter))
            {
                tmax = glm::max(params.scene_epsilon, light_sample.distance_to_light - params.scene_epsilon);
            }

            bool occluded = traceOcclusion(
                params.traversable_handle,
                si.position + params.scene_epsilon * light_sample.direction_to_light,
            ...
            output_radiance += ray_ctx.throughput * bsdf_eval.bsdf_value * light_sample.radiance_weight_at_receiver;
        }

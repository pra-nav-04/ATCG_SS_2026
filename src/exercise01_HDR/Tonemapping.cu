...
__global__ void scaleRgbKernel(glm::vec3* rgb_values, float scale, uint32_t number_pixels)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_pixels)
    {
    ...
    rgb_values[gid] *= scale;
}

__global__ void gammaCorrectRgbKernel(glm::vec3* rgb_values, float gamma, uint32_t number_pixels)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_pixels)
    {
    ...
    const glm::vec3 value = glm::max(rgb_values[gid], glm::vec3(0.0f));
    rgb_values[gid] = glm::pow(value, glm::vec3(gamma));
}

__device__ uint32_t histogramBinIndex(float brightness, float min_value, float max_value, uint32_t number_bins)
{
    ...
    const float normalized = glm::clamp((brightness - min_value) / (max_value - min_value), 0.0f, 1.0f);
    return glm::min(static_cast<uint32_t>(normalized * static_cast<float>(number_bins)), number_bins - 1);
}

__global__ void histogramBinsKernel(glm::vec3* hsv_values, uint32_t* bins, const float* min_value, const float* max_value, uint32_t number_pixels, uint32_t number_bins)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_pixels)
    {
    ...
    const uint32_t bin = histogramBinIndex(hsv_values[gid].z, *min_value, *max_value, number_bins);
    atomicAdd(bins + bin, 1);
}

__global__ void mapHistogramBrightnessKernel(glm::vec3* hsv_values, const float* accum_probs, const float* min_value, const float* max_value, uint32_t number_pixels, uint32_t number_bins)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_pixels)
    {
    ...
    glm::vec3 hsv = hsv_values[gid];
    const uint32_t bin = histogramBinIndex(hsv.z, *min_value, *max_value, number_bins);
    hsv.z = glm::clamp(accum_probs[bin], 0.0f, 1.0f);
    hsv_values[gid] = hsv;
}
...
void scaleRgb(glm::vec3* rgb, float scale, uint32_t number_pixels)
{
    ...
    scaleRgbKernel<<<block_count, block_size>>>(rgb, scale, number_pixels);
}
...
void gammaCorrectRgb(glm::vec3* rgb, float gamma, uint32_t number_pixels)
{
    const int block_size  = 512;
    const int block_count = ceil_div<int>(number_pixels, block_size);
    gammaCorrectRgbKernel<<<block_count, block_size>>>(rgb, gamma, number_pixels);
}
...
void histogramBins(glm::vec3* hsv_values, uint32_t* bins, const float* min_value, const float* max_value, uint32_t number_pixels, uint32_t number_bins)
{
    const int block_size  = 512;
    const int block_count = ceil_div<int>(number_pixels, block_size);
    histogramBinsKernel<<<block_count, block_size>>>(hsv_values, bins, min_value, max_value, number_pixels, number_bins);
}
...
void mapHistogramBrightness(glm::vec3* hsv_values, const float* accum_probs, const float* min_value, const float* max_value, uint32_t number_pixels, uint32_t number_bins)
{
    const int block_size  = 512;
    const int block_count = ceil_div<int>(number_pixels, block_size);
    mapHistogramBrightnessKernel<<<block_count, block_size>>>(hsv_values, accum_probs, min_value, max_value, number_pixels, number_bins);
}

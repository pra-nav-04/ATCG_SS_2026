...
__global__ void estimateIrradianceKernel(float* x_nom, float* x_denom, float* x, const uint8_t* values, const float* weights, const float* exposures, const float* I, uint32_t number_values, uint32_t values_per_image)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_values)
    {
    ...
    const uint32_t pixel_index = gid % values_per_image;
    const uint32_t image_index = gid / values_per_image;
    const float exposure = exposures[image_index];
    const float weight = weights[gid];
    const float response = I[values[gid]];

    atomicAdd(x_nom + pixel_index, weight * exposure * response);
    atomicAdd(x_denom + pixel_index, weight * exposure * exposure);

    if (gid < values_per_image)
    {
        x[gid] = 0.0f;
    }
}

__global__ void finalizeIrradianceKernel(float* x_nom, float* x_denom, float* x, uint32_t number_values)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_values)
    {
    ...
    const float denom = x_denom[gid];
    x[gid] = denom > 0.0f ? x_nom[gid] / denom : 0.0f;
}

__global__ void estimateInvCrfKernel(float* I_unnorm, const uint8_t* values, const float* weights, const float* exposures, const float* x, const bool* underexposed_mask, uint32_t number_values, uint32_t values_per_image)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_values)
    {
    ...
    const uint32_t pixel_index = gid % values_per_image;
    if (underexposed_mask[pixel_index])
    {
        return;
    }

    const uint32_t image_index = gid / values_per_image;
    const float contribution = weights[gid] * exposures[image_index] * x[pixel_index];
    atomicAdd(I_unnorm + values[gid], contribution);
}

__global__ void finalizeInvCrfKernel(float* I_unnorm, float* I, const uint32_t* counters, uint32_t number_values)
{
    const uint32_t gid = threadIdx.x + blockIdx.x * blockDim.x;
    if (gid >= number_values)
    {
    ...
    const uint32_t count = counters[gid];
    if (count > 0)
    {
        I[gid] = I_unnorm[gid] / static_cast<float>(count);
    }
}
...
void estimateIrradiance(float* x_nom, float* x_denom, float* x, const uint8_t* values, const float* weights, const float* exposures, const float* I, uint32_t number_values, uint32_t values_per_image)
{
    const int block_size_values = 512;
    const int block_count_values = ceil_div<int>(number_values, block_size_values);
    estimateIrradianceKernel<<<block_count_values, block_size_values>>>(x_nom, x_denom, x, values, weights, exposures, I, number_values, values_per_image);

    const int block_size_pixels = 512;
    const int block_count_pixels = ceil_div<int>(values_per_image, block_size_pixels);
    finalizeIrradianceKernel<<<block_count_pixels, block_size_pixels>>>(x_nom, x_denom, x, values_per_image);
}

void estimateInvCrf(float* I_unnorm, float* I, const uint8_t* values, const float* weights, const float* exposures, const float* x, const bool* underexposed_mask, const uint32_t* counters, uint32_t number_values, uint32_t values_per_image)
{
    const int block_size_values = 512;
    const int block_count_values = ceil_div<int>(number_values, block_size_values);
    estimateInvCrfKernel<<<block_count_values, block_size_values>>>(I_unnorm, values, weights, exposures, x, underexposed_mask, number_values, values_per_image);

    const int block_size_crf = 256;
    const int block_count_crf = ceil_div<int>(256, block_size_crf);
    finalizeInvCrfKernel<<<block_count_crf, block_size_crf>>>(I_unnorm, I, counters, 256);
}
